<script lang="ts">
	...
	export const getPostsClient = async (
		...
		const data = await fetch(
			`${backendUrl}/posts/users/${userId}?sort=${sort}&days=${days}&start=${offset}`,
			{
				headers: {
	...
	async function loadItems() {
		if (!$loaded) {
			$offset += 10;
			isLoading = true;
			const newPosts = await getPostsClient(userId, $offset, sortDays, sortType);
